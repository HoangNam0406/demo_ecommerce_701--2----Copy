...
// Display all products in Beyono brand (category_id = 2)
$products = [];
$stmt = $pdo->prepare('SELECT * FROM products WHERE category_id = 2 ORDER BY created_at DESC');
$stmt->execute();
    ...
    <div class="container">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0"> 
          ...
          <li class="breadcrumb-item active" aria-current="page">Beyono</li>
        </ol>
      </nav>
    </div>
   </div>
    ...
    <div class="container">
      <div class="row row-cols-1 row-cols-md-2 row-cols-lg-4 g-4">
        <?php if (!empty($products)): ?>
          ...
          <?php endforeach; ?>
        <?php else: ?>
          <div class="col-12"><div class="alert alert-danger">No Beyono products found.</div></div>
        <?php endif; ?>
      </div>
    </div>
  </section>
